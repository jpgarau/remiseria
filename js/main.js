...
$(function () {
	...
	if ($("#cmbTelegram").length > 0) {
		cargarTelegram();
	}
	if ($("#actTelegram").length > 0) {
		$("#actTelegram").on("click", async function () {
			let telid = $("#cmbTelegram").val();
			$("#actTelegram i").addClass("fa-spin");
			await actualizarTablaTelegram().then((exito) => {
...
function cargarTelegram() {
	return new Promise((exito) => {
		...
		let request = new Request("scripts/apitelegram.php", {
			method: "POST",
			body: datos,
		});
		fetch(request)
					...
					if (respuesta.encontrados > 0) {
						...
						if (arrTelegrams.length > 0) {
							arrTelegrams.forEach((telegram) => {
								...
								opTelegram.text =
									telegram.last_name + ", " + telegram.first_name;
								cmbTelegram.appendChild(opTelegram);
							});
...
function generarInformeVehiculo(servicios, vehiculo) {
	let total = 0.0,
		ctaCte = 0.0,
		recaudado = 0.0,
		viajes = 0;
